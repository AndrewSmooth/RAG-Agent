...
### 1. Активируйте Python-окружение
```bash
...
### 7. Запустите апи fastapi для загрузки файлов в chroma
```bash
sh run_api.sh
```
...
!NEW! Теперь можно добавлять файлы через фастапи по ручке /loader. Нужно загрузить файлы и указать название коллекции
...
| Recall@k \ Способ поиска | Hybrid Search | Hybrid with Prompting | Only Semantic Search |
|------------------------|---------------|----------------------|---------------------|
| Recall@5                      | 0.6           | 0.747                | 0.754               |
| Recall@20                     | 0.798         | 0.833                | 0.784               |
| Recall@50                     | 0.769         | 0.800                | 0.690               |